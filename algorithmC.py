def algorithmC(nodes):
    ...
    for ki in range(2, k):  # step 1
        node = nodes[ki]
        # change id to be the correct order in the list, so that I can easily track the nodes.
        node.id = ki
        node_low = node.low
        node_high = node.high
        cl = c[node_low.id]
        ch = c[node_high.id]
        # step 2
        c[ki] = pow(2, (node_low.value - node.value - 1)) * cl + \
                pow(2, (node_high.value - node.value - 1)) * ch
